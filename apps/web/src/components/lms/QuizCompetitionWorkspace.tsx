...
type PublicQuestion = {
  id: string; prompt: string; type: 'SINGLE_CHOICE' | 'MULTIPLE_CHOICE' | 'TRUE_FALSE'; options: Array<{ id: string; text: string }>;
  points: number; negativePoints: number; difficulty: 'EASY' | 'MEDIUM' | 'HARD'; topic?: string; sequence: number;
};
...
type ResultPayload = {
  score: number; totalMarks: number; percentage: number; correctCount: number; wrongCount: number; unansweredCount: number; submittedAt: string;
  released: boolean;
...
export function QuizCompetitionWorkspace({ courseId, quizId }: { courseId: string; quizId: string }) {
  ...
  React.useEffect(() => {
    ...
    void loadOverview().then((data) => {
      ...
      return undefined;
    }).catch((cause: unknown) => { if (!cancelled) setError(cause instanceof Error ? cause.message : 'Unable to load quiz competition.'); });
  ...
  React.useEffect(() => {
    if (!active || !attempt?.attempt.deadline) return;
    const deadlineMs = new Date(attempt.attempt.deadline).getTime();
  ...
  }, [active, attempt?.attempt.deadline]);

  const submitAttempt = React.useCallback(async (automatic = false) => {
    if (!attempt || attempt.attempt.completedAt || busy) return;
    setBusy(true); setError(''); setConfirmSubmit(false);
    try {
      const response = await fetch(attemptUrl(courseId, quizId, attempt.attempt.id), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'submit' }) });
      const payload: unknown = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(readError(payload, 'Unable to submit competition.'));
      await loadAttempt(attempt.attempt.id);
      await loadOverview();
      if (overview?.competition.leaderboardEnabled) void loadLeaderboard();
    } catch (cause: unknown) {
  ...
  }, [attempt, busy, courseId, quizId, loadAttempt, loadOverview]);

  React.useEffect(() => {
    if (active && secondsLeft === 0) void submitAttempt(true);
  }, [active, secondsLeft, submitAttempt]);
  ...
  const recordIntegrity = React.useCallback((eventType: 'TAB_HIDDEN' | 'WINDOW_BLUR' | 'FULLSCREEN_EXIT' | 'COPY_ATTEMPT' | 'PASTE_ATTEMPT') => {
    ...
    void fetch(attemptUrl(courseId, quizId, attempt.attempt.id), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'integrity', eventType }), keepalive: true }).catch(() => undefined);
  ...
  async function startAttempt() {
    ...
    try {
      ...
      await loadOverview();
    } catch (cause: unknown) { setError(cause instanceof Error ? cause.message : 'Unable to start competition.'); }
    finally { setBusy(false); }
  }
  ...
  async function saveAnswer(question: PublicQuestion, selectedOptionIds: string[]) {
    ...
    setSavingQuestionId(question.id); setError('');
    try {
  ...
  async function loadLeaderboard() {
    ...
    } catch (cause: unknown) { setError(cause instanceof Error ? cause.message : 'Unable to load leaderboard.'); }
  ...
  if (attempt.result) return <CompetitionResult courseId={courseId} quizId={quizId} overview={overview} attempt={attempt} leaderboard={leaderboard} leaderBusy={leaderBusy} onLeaderboard={() => void loadLeaderboard()} />;
    ...
    <div className={`${focusMode ? 'fixed inset-0 z-[100] overflow-y-auto bg-[#F4F7FB] p-3 sm:p-5' : 'space-y-5 pb-10'}`}>
      <header className="sticky top-0 z-30 rounded-2xl border border-[#173456] bg-[#0B1F3A] px-4 py-3 text-white shadow-[0_16px_35px_rgba(11,31,58,0.18)] sm:px-5">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button type="button" disabled={busy} onClick={() => setConfirmSubmit(true)} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 text-xs font-black text-[#0B1F3A] hover:bg-[#EEF4FC] disabled:opacity-50"><Send className="h-4 w-4" /> Submit</button>
        </div>
      ...
      {error && <div role="alert" className="flex items-start gap-3 rounded-xl border border-[#F0C4C0] bg-[#FFF4F2] px-4 py-3 text-sm font-semibold text-[#9F2D25]"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />{error}</div>}
...
function calculateSecondsLeft(deadline: string | null, serverNow: string) { if (!deadline) return null; const serverOffset = Date.now() - new Date(serverNow).getTime(); return Math.max(0, Math.ceil((new Date(deadline).getTime() + serverOffset - Date.now()) / 1000)); }
