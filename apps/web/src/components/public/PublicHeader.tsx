              ...
              <section key={group.title} className="min-w-0">
                ...
                <ul className="mt-3 space-y-1.5">
                  {group.links.map(([label, href]) => (
                    <li key={href}><DetailedMenuLink label={label} href={href} active={isActivePath(pathname, href)} close={close} /></li>
                  ))}
                </ul>
              </section>
            ))}
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Primary navigation" className="fixed inset-0 z-[120] flex flex-col bg-white xl:hidden">
      <div className="flex h-16 shrink-0 items-center justify-between border-b border-[#DDE5F0] px-4 sm:px-6">
        ...
        <button type="button" onClick={close} aria-label="Close navigation menu" className="inline-flex h-11 w-11 items-center justify-center rounded-xl text-[#344054] hover:bg-[#F2F4F7]"><X className="h-5 w-5" aria-hidden="true" /></button>
      </div>
          ...
          {menuGroups.map((item) => {
            ...
            return (
              <section key={item.label} className={`overflow-hidden rounded-2xl border ${expanded || active ? 'border-[#B8CCEF]' : 'border-[#DDE5F0]'}`}>
                ...
                {expanded && <div className="border-t border-[#E1E7EF] bg-[#FAFBFD] px-3 py-4"><Link href={item.href} onClick={close} className="mb-5 flex min-h-12 items-center justify-between rounded-xl bg-[#101D38] px-4 text-sm font-extrabold text-white">View {item.label.toLowerCase()} overview <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link><div className="space-y-6">{item.groups.map((group) => <section key={group.title}><h3 className="px-2 text-[10px] font-extrabold uppercase tracking-[0.11em] text-[#7C889A]">{group.title}</h3><ul className="mt-2 space-y-1.5">{group.links.map(([label, href]) => <li key={href}><DetailedMenuLink label={label} href={href} active={isActivePath(pathname, href)} close={close} /></li>)}</ul></section>)}</div></div>}
              </section>
            );
          })}
        </nav>
