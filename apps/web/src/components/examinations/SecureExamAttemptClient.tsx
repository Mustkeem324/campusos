...
function localDraftKey(questionId: string) {
...
function QuestionControl({ question, value, onChange, onBlur }: {
  ...
  if (question.questionType === 'SINGLE_CHOICE' || question.questionType === 'TRUE_FALSE') {
    ...
    return <div className="space-y-2">{choices.map((option, index) => {
      ...
      return <label key={`${text}-${index}`} className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-950"><input type="radio" name={question.id} checked={value === text} onChange={() => onChange(text)} onBlur={onBlur} className="mt-1" /><span className="text-sm leading-6">{text}</span></label>;
    })}</div>;
  }
  ...
  if (question.questionType === 'MULTIPLE_CHOICE') {
    ...
    return <div className="space-y-2">{options.map((option, index) => {
      ...
      return <label key={`${text}-${index}`} className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-950"><input type="checkbox" checked={checked} onChange={() => onChange(checked ? selected.filter((item) => item !== text) : [...selected, text])} onBlur={onBlur} className="mt-1" /><span className="text-sm leading-6">{text}</span></label>;
    })}</div>;
  }
...
export function SecureExamAttemptClient({ attemptId }: Props) {
  ...
  const load = React.useCallback(async () => {
    ...
    body.questions.forEach((question) => {
      if (question.answer !== null && question.answer !== undefined) restored[question.id] = question.answer;
        ...
        try {
          const draft = window.localStorage.getItem(localDraftKey(question.id));
  ...
  React.useEffect(() => {
    const connected = () => { setOnline(true); void api({ action: 'record_event', attemptId, source: 'NETWORK', eventType: 'NETWORK_RECONNECTED', severity: 'INFO' }).catch(() => undefined); };
    const disconnected = () => { setOnline(false); void api({ action: 'record_event', attemptId, source: 'NETWORK', eventType: 'NETWORK_DISCONNECTED', severity: 'LOW' }).catch(() => undefined); };
  ...
  React.useEffect(() => {
    ...
    void navigator.mediaDevices?.getUserMedia({ video: true, audio: false }).then((stream) => {
      ...
      stream.getVideoTracks().forEach((track) => track.addEventListener('ended', () => {
        void api({ action: 'record_event', attemptId, source: 'PRIMARY_CAMERA', eventType: 'CAMERA_DISCONNECTED', severity: 'MEDIUM' }).catch(() => undefined);
      }));
    }).catch(() => {
      void api({ action: 'record_event', attemptId, source: 'PRIMARY_CAMERA', eventType: 'CAMERA_PERMISSION_UNAVAILABLE', severity: 'MEDIUM' }).catch(() => undefined);
    });
  ...
  React.useEffect(() => {
    ...
    const visibility = () => {
      if (document.visibilityState === 'hidden') void api({ action: 'record_event', attemptId, source: 'SCREEN', eventType: 'TAB_HIDDEN', severity: 'LOW' }).catch(() => undefined);
    };
    const fullscreen = () => {
      if (!document.fullscreenElement) void api({ action: 'record_event', attemptId, source: 'SCREEN', eventType: 'FULLSCREEN_EXIT', severity: 'LOW' }).catch(() => undefined);
    };
  ...
  const saveOne = React.useCallback(async (questionId: string) => {
    ...
    setSaving(true);
    try {
      await api({ action: 'save_answer', attemptId, attemptQuestionId: questionId, answer: answers[questionId] ?? null, idempotencyKey: `${attemptId}:${questionId}:${crypto.randomUUID()}` });
      setDirty((current) => { const next = new Set(current); next.delete(questionId); return next; });
      try { window.localStorage.removeItem(localDraftKey(questionId)); } catch { /* best effort */ }
    } finally {
      setSaving(false);
    }
  ...
  const submit = React.useCallback(async (auto = false) => {
    ...
    try {
      ...
      await api({ action: 'submit_attempt', attemptId, auto });
  ...
  function updateAnswer(questionId: string, value: unknown) {
    ...
    try { window.localStorage.setItem(localDraftKey(questionId), JSON.stringify(value)); } catch { /* storage can be unavailable */ }
  }
