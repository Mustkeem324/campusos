...
const DEFAULT_PHASE2_SETTINGS: TransportPhase2Settings = {
...
function settingsFromRow(row?: SettingsRow): TransportPhase2Settings {
  if (!row) return DEFAULT_PHASE2_SETTINGS;
  return {
...
async function readSettings(tenantId: string) {
  try {
    ...
    return {
      storeReady: true,
      moduleEnabled: rows[0]?.enabled ?? false,
      allowHybridStudents: rows[0]?.allow_hybrid_students ?? true,
      settings: settingsFromRow(rows[0]),
    };
  } catch (error) {
...
function stopFromRow(row: StopRow): TransportRouteStop {
  return {
    id: row.id,
...
async function readStops(tenantId: string, routeId?: string): Promise<TransportRouteStop[]> {
  try {
    const rows = routeId
...
export function estimateEtaMinutes(distanceM: number, speedKph: number | null | undefined, fallbackSpeedKph: number): number {
  ...
  // A small road-network factor keeps straight-line distance from creating
  // unrealistically optimistic ETAs without requiring a third-party maps API.
...
async function journeysFor(tenantId: string, vehicleIds: string[] | null): Promise<TransportJourneyInsight[]> {
  const [rows, stops] = await Promise.all([readJourneyRows(tenantId, vehicleIds), readStops(tenantId)]);
...
export async function getTransportPhase2LiveData(): Promise<TransportPhase2LiveData> {
  ...
  const settingsState = await readSettings(context.tenantId);
  if (!settingsState.storeReady || !settingsState.moduleEnabled) {
    return { generatedAt: new Date().toISOString(), enabled: false, settings: settingsState.settings, journeys: [] };
  }
  const vehicleIds = await allowedVehicleIds(context, settingsState.allowHybridStudents);
  const journeys = await journeysFor(context.tenantId, vehicleIds);
  return { generatedAt: new Date().toISOString(), enabled: true, settings: settingsState.settings, journeys };
}
...
export async function getTransportPhase2AdminData(): Promise<TransportPhase2AdminData> {
  ...
  const [routes, stops, journeys] = await Promise.all([
    ...
    readStops(context.tenantId),
    journeysFor(context.tenantId, null),
  ]);
  return {
    generatedAt: new Date().toISOString(),
    enabled: settingsState.moduleEnabled,
...
export async function updateTransportPhase2Settings(input: Partial<TransportPhase2Settings>) {
  const context = await requireActiveUserContext();
  requireInstitutionAdmin(context);
  const state = await readSettings(context.tenantId);
...
export async function createTransportRouteStop(input: {
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
      throw new TransportPhase2Error('This route already has a stop at that sequence number.', 409);
    }
    throw error;
  }
...
export async function processTransportPhase2Telemetry(input: {
  ...
  const state = await readSettings(input.tenantId);
  ...
  const stops = await readStops(input.tenantId, vehicle.route_id);
  ...
  if (stops.length === 0) {
    await prisma.$executeRaw`
      ...
      ON CONFLICT (vehicle_id) DO UPDATE SET route_id = EXCLUDED.route_id, service_date = EXCLUDED.service_date,
        ...
        next_stop_id = NULL, distance_to_next_m = NULL, eta_minutes = NULL, predicted_arrival_at = NULL,
        delay_minutes = NULL, journey_status = 'NO_STOPS', updated_at = now()
    `;
  ...
  const continuing = current && currentDay === day && current.journey_status !== 'COMPLETED';
  const tripStartedAt = continuing ? new Date(current.trip_started_at) : input.recordedAt;
  ...
  if (distanceM <= stop.geofenceRadiusM) {
    await prisma.$executeRaw`
      ...
      VALUES
        (${randomUUID()}::uuid, ${input.tenantId}::uuid, ${vehicle.id}::uuid, ${vehicle.route_id}::uuid,
    ...
    if (!next) {
      await prisma.$executeRaw`
        ...
        VALUES
          (${vehicle.id}::uuid, ${input.tenantId}::uuid, ${vehicle.route_id}::uuid, ${day}::date, ${tripStartedAt}, NULL,
           ${stop.id}::uuid, NULL, NULL, NULL, 0, 'COMPLETED', now())
        ON CONFLICT (vehicle_id) DO UPDATE SET service_date = EXCLUDED.service_date, trip_started_at = EXCLUDED.trip_started_at,
          next_stop_id = NULL, last_stop_id = EXCLUDED.last_stop_id, distance_to_next_m = NULL, eta_minutes = NULL,
          predicted_arrival_at = NULL, delay_minutes = 0, journey_status = 'COMPLETED', updated_at = now()
      `;
  ...
  await prisma.$executeRaw`
    INSERT INTO campusos_transport.vehicle_trip_state
      (vehicle_id, tenant_id, route_id, service_date, trip_started_at, next_stop_id, last_stop_id,
       distance_to_next_m, eta_minutes, predicted_arrival_at, delay_minutes, journey_status, updated_at)
    VALUES
      (${vehicle.id}::uuid, ${input.tenantId}::uuid, ${vehicle.route_id}::uuid, ${day}::date, ${tripStartedAt},
    ...
    ON CONFLICT (vehicle_id) DO UPDATE SET
      ...
      last_stop_id = COALESCE(EXCLUDED.last_stop_id, campusos_transport.vehicle_trip_state.last_stop_id),
