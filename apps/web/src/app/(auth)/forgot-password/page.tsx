    ...
    try {
      const response = await fetch('/api/auth/password/forgot', {
        ...
        body: JSON.stringify({
          ...
          workspace: String(form.get('workspace') || '').trim().toLowerCase(),
        }),
      });
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#F4F7FB] p-4 dark:bg-[#090D16] sm:p-6">
      <section className="w-full max-w-lg overflow-hidden rounded-[28px] border border-[#D8E2EF] bg-white shadow-[0_28px_80px_rgba(16,29,56,0.12)] dark:border-slate-800 dark:bg-slate-950">
        <header className="bg-[#101D38] p-6 text-white sm:p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-[#B8C6D9]">Navemora queues a time-limited reset link without revealing whether an account exists.</p>
        </header>
        <div className="p-6 sm:p-8">
          <form onSubmit={submit} className="space-y-4">
            ...
            <label className="block text-xs font-extrabold uppercase tracking-[0.09em] text-[#526175] dark:text-slate-400">Institution workspace <span className="normal-case tracking-normal text-[#8A95A6]">(optional)</span>
              <div className="relative mt-2"><Building2 className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-[#8A95A6]" /><input name="workspace" type="text" autoCapitalize="none" autoCorrect="off" spellCheck={false} className="min-h-11 w-full rounded-xl border border-[#CFD9E7] bg-white pl-10 pr-3 text-sm font-semibold text-[#101D38] outline-none focus:border-[#1754E8] focus:ring-2 focus:ring-[#1754E8]/15 dark:border-slate-700 dark:bg-slate-900 dark:text-white" placeholder="your-institution" /></div>
              <span className="mt-2 block text-xs font-medium normal-case tracking-normal text-[#8A95A6]">Use this when the same email belongs to more than one institution.</span>
            </label>
            <button type="submit" disabled={loading} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#1754E8] px-4 text-sm font-extrabold text-white hover:bg-[#103FC2] disabled:opacity-60">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}Send reset instructions</button>
          </form>
