import { NextResponse } from 'next/server';
import { requireTenantContext } from '../../../../../../lib/tenant-context';
import { z } from 'zod';

export const dynamic = 'force-dynamic';

const reactionSchema = z.object({
  type: z.enum(['LIKE', 'HELPFUL', 'INSIGHTFUL', 'AGREE', 'CELEBRATE', 'SUPPORT']),
});

export async function POST(request: Request, { params: paramsPromise }: { params: Promise<{ id: string }>; }) {
  const params = await paramsPromise;

  try {
    const { db, session } = await requireTenantContext();
    const postId = params.id;

    const post = await db.communityPost.findUnique({ where: { id: postId }, select: { id: true } });
    if (!post) return NextResponse.json({ error: 'Post not found' }, { status: 404 });

    const body = await request.json();
    const data = reactionSchema.parse(body);

    // Toggle: if reaction exists, remove it. Otherwise, create it.
    const existing = await db.communityReaction.findFirst({
      where: { postId, userId: session.userId, type: data.type },
    });

    if (existing) {
      await db.communityReaction.delete({ where: { id: existing.id } });
      return NextResponse.json({ action: 'removed', type: data.type });
    }

    await db.communityReaction.create({
      data: {
        postId,
        userId: session.userId,
        type: data.type,
        tenantId: session.tenantId,
      },
    });

    return NextResponse.json({ action: 'added', type: data.type }, { status: 201 });
  } catch (error: unknown) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: 'Invalid reaction type' }, { status: 400 });
    }
    const status = error instanceof Error && error.message.startsWith('Unauthorized') ? 401 : 500;
    return NextResponse.json({ error: status === 401 ? 'Unauthorized' : 'Failed to update reaction' }, { status });
  }
}
